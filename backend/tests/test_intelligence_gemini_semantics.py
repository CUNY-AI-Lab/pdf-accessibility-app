...
from app.services.intelligence_gemini_semantics import adjudicate_semantic_unit, adjudicate_semantic_units
...
def test_adjudicate_semantic_units_preserves_order(monkeypatch, tmp_path):
    ...
    decisions = asyncio.run(adjudicate_semantic_units(job=_job(tmp_path), units=units, llm_client=llm))
...
def test_adjudicate_semantic_unit_repairs_missing_required_payload(monkeypatch, tmp_path):
    ...
    unit = SemanticUnit(
        ...
        bbox={"l": 10, "t": 20, "r": 50, "b": 40},
        current_semantics={"field_name": "c1_37[0]", "accessible_label": "", "field_type": "checkbox"},
    )
    decision = asyncio.run(adjudicate_semantic_unit(job=_job(tmp_path), unit=unit, llm_client=llm))
